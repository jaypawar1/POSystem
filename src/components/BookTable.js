...
const BookTable = ({ tables, onSelectTable }) => {
    return (
        <div className="w-screen min-h-screen">
            ...
            <div className="w-screen min-h-[65vh]">
                {tables.map((table, index) => (
                    <div key={index}>
                        <h1 className="mx-3 my-3 font-bold text-xl text-[#8a6240]">
                            {table.catagory.toUpperCase()}
                        </h1>
                        <div className="w-screen h-[15vh] flex">
                            {table.table.map((item, idx) => (
                                <div key={idx} className="h-[15vh] w-[15vh] rounded-xl border-2 border-gray-500 border-dashed flex justify-center items-center bg-zinc-300 mx-2 cursor-pointer" onClick={()=>onSelectTable(item)}>{item.number}</div>
                            ))}
                        </div>
                    </div>
                ))}
